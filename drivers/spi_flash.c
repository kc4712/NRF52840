...
int spi_send_recv(uint8_t * const p_tx_data,
...
#ifndef GC_NRF52_SPI_FLASH_INT
		err_code = nrf_drv_ppi_channel_disable(ppi_channel_workaround);
		if (err_code != NRF_SUCCESS)
			printf("\r\nPPI-ch disable error(code %d)", err_code);
				
		err_code = nrf_drv_ppi_channel_free(ppi_channel_workaround);
		if (err_code != NRF_SUCCESS)
			printf("\r\nPPI-ch free error(code %d)", err_code);
...
#else
	err_code = nrf_drv_spi_init(&spi_flash, &spi_flash_config, NULL);
#endif 
...
#ifdef GC_NRF52_SPI_FLASH_INT
	gc_flash_status = GC_FLASH_WRITE_ENABLE_TO_ERASE;
	gc_flash_sectoraddr = SectorAddr;
#endif 	
...
int32_t SPI_FLASH_PageWrite(unsigned char* pBuffer, unsigned int WriteAddr)
{
	...
	err_code = SPI_FLASH_WriteEnable();
	if (err_code != NRF_SUCCESS)
		return err_code;
	...
	return spi_send_recv(pBuffer, rx_data, 255); 
#elif defined GC_NRF51_SPI_FLASH
